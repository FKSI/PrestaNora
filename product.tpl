...
<div id="primary_block" class="clearfix">
	...
	<div id="pb-right-column">
		...
		<ul id="usefull_link_block">
			...
			<li id="print_it"><a href="javascript:print();">{l s='Print'}</a><br class="clear" /></li>
			{if $have_image && !$jqZoomEnabled}
			<li id="zoom_it"><span id="view_full_size" class="span_link">{l s='View full size'}</span></li>
			{/if}
		</ul>
		<!-- product img-->
		<div id="image-block">
	...
	<h2>{$product->name|escape:'htmlall':'UTF-8'}</h2>
		...
		<div id="short_description_block">
			...
			<p class="buttons_bottom_block"><a href="javascript:{ldelim}{rdelim}" class="custom button">{l s='More details'}</a></p>
			{/if}
		...
		<form id="buy_block" action="{$base_dir}cart.php" method="post">
			...
			<p id="pQuantityAvailable"{if $display_qties != 1 || ($allow_oosp && $product->quantity == 0)} style="display:none;"{/if}>
				...
				<span id="quantityAvailable" class="bold">{$product->quantity|intval}</span>
			</p>			
			</div>
			...
			<p class="warning-inline" id="last_quantities"{if ($product->quantity > $last_qties || $product->quantity == 0) || $allow_oosp} style="display:none;"{/if} >{l s='Last items in stock!'}</p>

			<p{if !$allow_oosp && $product->quantity == 0} style="display:none;"{/if} id="add_to_cart" class="buttons_bottom_block"><input type="submit" name="Submit" value="{l s='Add to cart'}" class="custom" /></p>
