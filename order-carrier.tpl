...
<form id="form" class="shipping_option" action="{$base_dir_ssl}order.php" method="post" onsubmit="return acceptCGV('{l s='Please accept the terms of service before the next step.' js=1}');">
    <h3>{l s='Shipping Carrier'}</h3>
	{if $carriers && count($carriers)}
		<table class="std shipping_carrier">
			<thead>
				<tr>
...
{else}
	<h6>{l s='Choose your delivery method'}</h6>
...
{/if}
	<p class="cart_navigation">
		...
		<a href="{$base_dir_ssl}order.php?step=1{if $back}&back={$back}{/if}" title="{l s='Previous'}" class="button left">&laquo; {l s='Previous'}</a>
