...
<p class="modify_address">{if isset($id_address)}{l s='Modify the address'} {if isset($smarty.post.alias)}"{$smarty.post.alias}"{elseif $address->alias}"{$address->alias|escape:'htmlall':'UTF-8'}"{/if}{else}{l s='To add a new address, please fill out the form below.'}{/if}</p>
...
<form action="{$base_dir_ssl}address.php" method="post" class="std">
	<fieldset>
		...
		<p class="required">
			<label for="postcode">{l s='Postal code :'}</label>
		...
		<p class="required" id="adress_alias">
