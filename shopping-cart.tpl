	...
	<form action="{$base_dir_ssl}order.php" method="post" id="voucher">
		<fieldset>
			<p>
				...
				<input type="submit" name="submitDiscount" value="{l s='Add'}" class="button special" /></p>
		</fieldset>
	</form>
</div>
{/if}
...
<p class="cart_navigation">
	<a href="{if $smarty.server.HTTP_REFERER && strstr($smarty.server.HTTP_REFERER, 'order.php')}{$base_dir}index.php{else}{$smarty.server.HTTP_REFERER|escape:'htmlall':'UTF-8'|secureReferrer}{/if}" class="button left" title="{l s='Continue shopping'}">&laquo; {l s='Back'}</a>
	<a href="{$base_dir_ssl}order.php?step=1{if $back}&amp;back={$back}{/if}" class="button right" title="{l s='Next'}">{l s='Next'} &raquo;</a>
</p>
