...
<form action="{$base_dir_ssl}order.php" method="post">
	<div class="addresses">
		...
		{if $addresses|@count > 1}
			...
			{else}
				<a style="margin-left: 221px;" href="{$base_dir_ssl}address.php?back=order.php&amp;step=1&select_address=1{if $back}&mod={$back}{/if}" title="{l s='Add'}" class="button special">{l s='Add a new address'}</a>
			{/if}
		</p>
		<div class="clear"></div>
		<ul class="address item" id="address_delivery">
			...
			<li class="address_update custom"><a href="{$base_dir_ssl}address.php?id_address={$address.id_address|intval}&amp;back=order.php&amp;step=1{if $back}&mod={$back}{/if}" title="{l s='Update'}">{l s='Update'}</a></li>
		</ul>
		...
		<br class="clear" />
		<p>
			<a href="{$base_dir_ssl}address.php?back=order.php&amp;step=1{if $back}&mod={$back}{/if}" title="{l s='Add'}" class="button special">{l s='Add a new address'}</a>
		</p>
		<div id="ordermsg">
	...
	<p class="cart_navigation">
		...
		<a href="{$base_dir_ssl}order.php?step=0{if $back}&back={$back}{/if}" title="{l s='Previous'}" class="button left">&laquo; {l s='Previous'}</a>
